...
MAIN_MENU() {
  ...
  case $MAIN_MENU_SELECTION in #case redirects to de appropiate funtion, * make reference to any other value
    1) RENT_MENU ;;
    2) RETURN_MENU ;;
    3) EXIT ;;
...
RENT_MENU() {
  ...
  if [[ -z $AVAILABLE_BIKES ]] # Evaluate if the variable is empty, this happend if there is not available bikes
  then
    # send to main menu
    MAIN_MENU "Sorry, we don't have any bikes available right now."
    
  else # Is considered just where the query retunr any valor of available bikes
    # display available bikes
    echo -e "\nHere are the bikes we have available:"
    echo "$AVAILABLE_BIKES" | while read BIKE_ID BAR TYPE BAR SIZE # the pipe (|) get as stdout(output) each of the values result from the query and tranform them into variables
    do
    ...
    if [[ ! $BIKE_ID_TO_RENT =~ ^[0-9]+$ ]] #Evaluate if the value is a valid number (!) the contray to the regrex characters
    then
      # send to main menu
